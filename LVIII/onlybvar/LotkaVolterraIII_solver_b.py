...
#b = 0.001 # interaction rate between prey and predator
m = 0.1 # predator eating themselves
K = 500 # prey limit
k = 0.2 # predator limit
t_h = 100*h

def f( t, y, F, b ): # F is arbitrary function, but should be fitted to the RK4 solver
    F[0] = a*y[0]*( 1 - y[0]/K ) - (b*y[0]*y[1])/(1+b*y[0]*t_h ) # RHS of 1st eq
    F[1] = m*y[1]*( 1 - (y[1]/(k*y[0])) )                        # RHS of 2nd eq

# -------------------------Start of RK4 solver--------------------------------
def rk4( t, y, h, Neqs, b ): # RK4 solver for arbitrary F; Neqs is the #eqs
    ...
    f( t, y, F, b ) # function evaluation at beginning point of the interval
    h2 = h*0.5
    for i in range( 0, Neqs ): # Euler-step
        k1[i] = h*F[i] # slope at the beginning of the interval using y
        ytemp[i] = y[i] + k1[i]*0.5
    f( t+h2, ytemp, F, b ) # function evaluation at midpoint
    ...
    for i in range( 0, Neqs ): # 3rd RK4-step
        ...
        ytemp[i] = y[i] + k3[i]
    f( t+h, ytemp, F, b ) # function evaluation at the end point of the interval
...
for i in range(1):
    #b += 0.001
#    c = float("{0:.3f}".format(b))
#    str3 = str(c)
#    str3 = str3[2:]
    str3 = str(b*10)
#    str3 = str3[:-2]
    file = open( 'LVIII_p'+str1+'_P'+str2+'b'+str3+'.dat', 'w' ) # create file to write out
    file.write("#time\tprey_t\tpredator_t\n")
    while t < (Tmax):
        file.write( "%f %f %f\n" % (t, y[0], y[1]) )
        rk4( t, y, h, 2, b ) # running the solver
        t += h
    t=Tmin
    y[0] = 2.0
    y[1] = 1.3
